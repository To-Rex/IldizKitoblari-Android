...
class AudioPagePreview extends StatefulWidget {
  final String sId;
  final String title;

  const AudioPagePreview({super.key, required this.sId, required this.title});

  @override
  _AudioPlayerBackgroundPlaylistState createState() => _AudioPlayerBackgroundPlaylistState();
}

class _AudioPlayerBackgroundPlaylistState extends State<AudioPagePreview> {
  ...
  Future<void> _loadAudio({bool isRefresh = false}) async {
    try {
      ...
      await ApiController().getBookAudio(widget.sId);
      ...
      setState(() {
        audioUrl = _getController.productAudioModel.value.data?.result?.isNotEmpty == true
            ? _getController.productAudioModel.value.data!.result![0].file?.path
            : null;
      });
    } catch (e) {
  ...
  void initState() {
    ...
    audioPlayer.processingStateStream.listen((state) async {
      if (state == ProcessingState.completed) {
        final resultsLength = _getController.productAudioModel.value.data?.result?.length ?? 1;
        final nextIndex = (selectedAudio + 1) % resultsLength;
        if (nextIndex < lockedStatus.length && lockedStatus[nextIndex]) {
          await audioPlayer.pause();
          setState(() {
            isPlaying = false;
          });
          debugPrint('Qulflangan trekka o‘tish oldi olindi: index=$nextIndex');
        } else {
          await audioPlayer.seek(Duration.zero, index: nextIndex);
          await audioPlayer.play();
          debugPrint('Keyingi trekka o‘tildi: index=$nextIndex');
        }
      }
    });
  }
  ...
  Future<void> setupPlaylist() async {
    try {
      final results = _getController.productAudioModel.value.data?.result;
      ...
      for (var result in results) {
        final isLocked = result.locked ?? true;
        lockedStatus.add(isLocked);
        if (!isLocked && result.file?.path != null && result.file!.path!.isNotEmpty) {
          sources.add(
            AudioSource.uri(
              Uri.parse(result.file!.path!),
              tag: {
      ...
      if (sources.isEmpty) {
        debugPrint('Qulflanmagan audio topilmadi');
        setState(() {
          playlist = null;
          audioUrl = null;
        });
  ...
  Future<void> skipPrevious() async {
    try {
      final prevIndex = (selectedAudio - 1) >= 0
          ? selectedAudio - 1
          : (_getController.productAudioModel.value.data?.result?.length ?? 1) - 1;
      if (prevIndex < lockedStatus.length && lockedStatus[prevIndex]) {
        await audioPlayer.pause();
        setState(() {
          isPlaying = false;
        });
        debugPrint('Qulflangan oldingi trekka o‘tish oldi olindi: index=$prevIndex');
      } else {
        await audioPlayer.seekToPrevious();
        debugPrint('Oldingi trekka o‘tildi: index=$prevIndex');
      }
    } catch (e) {
  ...
  Future<void> skipNext() async {
    try {
      ...
      if (nextIndex < lockedStatus.length && lockedStatus[nextIndex]) {
        ...
        debugPrint('Qulflangan keyingi trekka o‘tish oldi olindi: index=$nextIndex');
      } else {
  ...
  Future<void> _playSelectedAudio(int index) async {
    if (_getController.productAudioModel.value.data?.result?.isNotEmpty == true &&
        _getController.productAudioModel.value.data!.result![index].locked == true) {
      ApiController().showToast(context, 'Xatolik', 'Bu audio qulflangan!', true, 3);
      return;
    }
            ...
            builder: (context, snapshot) {
              ...
              return Column(
                ...
                children: [
                      ...
                      children: [
                        Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            image: DecorationImage(
                              image: NetworkImage(
                                _getController.productAudioModel.value.data?.product?.image ??
                        ...
                        TextLarge(
                          text: _getController.getLocale() == 'uz_UZ'
                              ? _getController.productAudioModel.value.data?.product?.name?.uz ?? ''
                              : _getController.getLocale() == 'oz_OZ'
                              ? _getController.productAudioModel.value.data?.product?.name?.oz ?? ''
                              : _getController.productAudioModel.value.data?.product?.name?.ru ?? '',
                        ...
                        Row(
                          children: [
                            SizedBox(width: 15.w),
                            TextSmall(
                              text: _getController.productAudioModel.value.data?.result?.isNotEmpty == true &&
                                  selectedAudio < (_getController.productAudioModel.value.data?.result?.length ?? 0)
                                  ? _getController.getLocale() == 'uz_UZ'
                                  ? _getController.productAudioModel.value.data!.result![selectedAudio].name?.uz ?? 'Noma’lum'
                                  : _getController.getLocale() == 'oz_OZ'
                                  ? _getController.productAudioModel.value.data!.result![selectedAudio].name?.oz ?? 'Noma’lum'
                                  : _getController.productAudioModel.value.data!.result![selectedAudio].name?.ru ?? 'Noma’lum'
                                  : 'Noma’lum audio',
                              fontSize: 20.sp,
                              fontWeight: FontWeight.bold,
                              color: Theme.of(context).colorScheme.onSurface,
                  ...
                  SizedBox(height: 10.h),
                  for (int index = 0; index < (_getController.productAudioModel.value.data?.result?.length ?? 0); index++)
                    Column(
                      children: [
                        InkWell(
                            ...
                            child: Row(
                              ...
                              children: [
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      TextSmall(
                                        text: _getController.productAudioModel.value.data?.result?.isNotEmpty == true
                                            ? _getController.getLocale() == 'uz_UZ'
                                            ? _getController.productAudioModel.value.data!.result![index].name?.uz ?? 'Noma’lum'
                                            : _getController.getLocale() == 'oz_OZ'
                                            ? _getController.productAudioModel.value.data!.result![index].name?.oz ?? 'Noma’lum'
                                            : _getController.productAudioModel.value.data!.result![index].name?.ru ?? 'Noma’lum'
                                            : 'Noma’lum audio',
                                      ...
                                      SizedBox(height: 5.h),
                                      if (_getController.productAudioModel.value.data?.result?.isNotEmpty == true &&
                                          _getController.productAudioModel.value.data!.result![index].file != null)
                                        TextSmall(
                                          text: '${(_getController.productAudioModel.value.data!.result![index].file!.size! / 1024 / 1024).toStringAsFixed(2)} MB',
                                          color: selectedAudio == index ? AppColors.primaryColor : Theme.of(context).colorScheme.onSurface,
                                        ),
                                    ],
                                  ),
                                ...
                                Container(
                                  ...
                                  child: IconButton(
                                    icon: Icon(
                                      _getController.productAudioModel.value.data?.result?.isNotEmpty == true &&
                                          _getController.productAudioModel.value.data!.result![index].locked == true
                                    ...
                                    onPressed: _getController.productAudioModel.value.data?.result?.isNotEmpty == true &&
                                        _getController.productAudioModel.value.data!.result![index].locked == true
                                        ? null
                                        : () => _playSelectedAudio(index),
                                  ),
                                ),
                              ],
                            ),
                        ...
                        if (index != (_getController.productAudioModel.value.data?.result?.length ?? 1) - 1)
                          Divider(
                            thickness: 1,
